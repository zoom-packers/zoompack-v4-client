let stunPowerMultiplyer = 0.0001;
let stunBaseRange = 10;

function stunRange(base, spell_power, evocation_spell_power){
    return base+1+1*spell_power*evocation_spell_power*stunPowerMultiplyer;
}
    ...
    event.create('irons_spellbooks:stun')
        ...
        .setUniqueInfo((spellLevel, caster) => {
            try{
                ...
                return [
                    Component.green(`Effect Duration: ${Math.floor(2+1 * 1 * spellLevel*stunPowerMultiplyer)} s`),
                    Component.green(`Range: ${Math.floor(stunRange(stunBaseRange, 1, 1))} m`),
                ]
            }
